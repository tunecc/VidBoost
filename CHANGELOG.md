# Changelog

本文档记录 VidBoost 的版本更新历史。

整理依据：

- 已发布的 GitHub Releases 公告
- 各版本 tag 区间内的真实 commits
- 仓库内历史 release 公告草稿与发布说明文档

## [1.9.2] - 2026-09-21

1.9.1 把 VidBoost 自己的字幕选择器放进播放器之后，1.9.2 集中解决「找不到入口」和「选不准语言」：只要打开自渲染字幕，控件栏就一直有 VidBoost 的按钮（当前没在渲染就置灰），不必先去点 YouTube 的 CC 按钮；自动翻译分组把简体、繁体中文顶到最前，常用语言不用再翻。同时补了一轮绑定与稳定性加固：字幕不会再跟到推荐卡片的预览视频，面板打开时也不会被轮询重建。

### Added
- **控件栏字幕按钮常驻与置灰两态**（`YouTubeSubtitleOverlay` / `SubtitleSelector`）  
  开启「自渲染字幕」后，观看页控件栏始终挂载 VidBoost 按钮。原生 CC 关闭、视频无任何字幕、命中无字幕缓存、拉取失败回退原生字幕、语言目录尚未取到这几种「当前没有字幕在渲染」的状态下，图标改为置灰而不是整个消失。
- **CC 关闭时直接从 VidBoost 面板开启字幕**  
  置灰状态下也会取一次语言目录；选中语言后先把 YouTube 原生 CC 置为开启再渲染，图标转为高亮。从面板显式开启后，本视频不再被「记住 YouTube 字幕开关」的恢复逻辑撤回。
- **「该视频暂无字幕」空状态**  
  新增中英文案（`yt_subtitle_empty_catalog`）：视频确实没有任何字幕时，点开面板给出提示并收起搜索入口。
- **无字幕视频缓存**  
  标记后不再为同一视频重复发起字幕请求。

### Changed
- **自动翻译分组置顶简体中文、繁体中文**（`catalog.ts`）  
  按语言码判定书写组，YouTube 界面为英文时顺序一致；其余语言按标签升序，比较固定使用 `zh-Hans` locale，不随浏览器 UI 语言漂移。
- **搜索行收进标题行的搜索按钮**  
  默认不显示，点击才展开，收起即清空过滤条件；`Esc` 先收起搜索行、再关闭面板。
- **面板视觉贴合 YouTube 原生深色菜单**  
  去掉外边框与强阴影，改实色深底、整行 hover、选中项淡蓝底加对号，徽章弱化为小字。
- **字幕加载失败阈值调整为 3 次**  
  前 2 次先重试，避免单次偶发失败就立刻回退到原生字幕。

### Fixed
- **字幕不再绑到推荐卡片的预览视频**  
  覆盖层始终跟随主播放器的 `<video>`，鼠标悬停推荐视频时字幕不会跳走。
- **菜单打开时不再误触切换字幕**  
  检测到播放器弹层可见时不再点击 CC 按钮。
- **面板打开期间不再被轮询重建**  
  渲染内容（语言项与顺序、选中项、偏好语言、查询词、搜索展开态、全部文案）没有变化时不动列表 DOM，用方向键选语言的焦点、列表滚动位置与悬停高亮不再每 1.2 秒被清掉；弹层定位仍照常更新。

## [1.9.1] - 2026-08-17

1.9.0 让 YouTube 自渲染字幕在「兼容沉浸式翻译」上更克制后，1.9.1 把重心收回到**字幕本身的可达性**：在播放器内嵌一个 VidBoost 自己的字幕语言选择器，让你不必绕回 YouTube 原生菜单，也能直接切换 / 记住目标语言；并配套一轮清理与图标打磨。

### Added
- **[新增] 播放器内字幕选择器**（`SubtitleSelector`）  
  在 YouTube 播放器上挂载 VidBoost 自己的字幕按钮与下拉菜单：搜索框 + 选项列表，直接切语言，不必再回到 YouTube 原生 CC 菜单。含菜单状态机（`menuState`）、i18n 文案与对应单测。
- **[新增] 单字幕目标语言请求管线**  
  `catalog` / `language` / `request` / `resolver` 一组工具：构建单字幕语言目录、规范化目标语言码、解析单字幕目标请求并落到对应 track；`YouTubeSubtitleOverlay` 接入后改为按 `SubtitleOption` 取 baseUrl 与渲染。
- **[新增] 记住目标字幕语言**  
  设置新增持久化字段，下次打开同语言视频时优先恢复你上次选择的目标语言。
- **[新增] 读取 YouTube 翻译语言**  
  解析 YouTube 现有的翻译字幕信息，作为选择器候选项来源之一。
- **[新增] 字幕选择器图标测试**（`icon.test.ts`，8 个用例）  
  覆盖激活态彩色 / 未激活态灰度半透明、图标尺寸与可访问性等验收项。

### Changed
- **字幕选择器按钮图标改用 VidBoost 主图标**  
  从 `CC` 文本框换为 `icons/icon48.png`（缩放至 24px）；图标随字幕激活态变化：激活时彩色，未激活时灰度半透明。`manifest.json` 的 `web_accessible_resources` 同步放开 `icons/*`，并新增 `runtimeGetURL` 导出别名。
- **字幕请求模型重构**  
  `buildSubtitleUrl` 改为接收 `SubtitleOption`，由 `option.sourceTrack` 取 baseUrl；旧的 `selectTrack` / `buildTrackKey` 等 track 直选逻辑被选择器管线取代。
- 移除冗余的设置转发层与无效运行时接口，清理未使用的历史文件；固定本地测试工具依赖。
- 版本号升至 `1.9.1`（`package.json` / `package-lock.json` / `public/manifest.json`）。

### Fixed
- 修复播放器字幕选择器挂载失败的问题（按钮挂载点解析与重试逻辑）。
- 修正字幕选择器去重菜单的选中态显示。

### Notes
- 相关 commits（`v1.9.0` → HEAD）：`4a3e338`、`984bda8`、`ae42f4f`、`8de9a76`、`d5b0eee`、`655c3ef`、`ea7cbad`、`4634be2`、`01369ba`、`ddda3d9`、`f547363`、`034b6e1`、`29bf250`、`f600cef`、`8ce1701`。

## [1.9.0] - 2026-08-11

1. **可让沉浸式翻译渲染非中文字幕** — 开启兼容后，非中文字幕交给 IT 显示翻译结果，VidBoost 不再叠加渲染；中文字幕仍由 VidBoost 强化渲染。
2. **字幕优化更聪明** — 不再依赖不可靠的 `track.kind` 标签，改为按内容形态判断：精修短语直接透传，只有碎 cue 才做重切精炼。
3. **Popup 主页面更紧凑** — 头部压缩、间距收紧，同屏信息密度更高。

### Added
- **[新增] 兼容沉浸式翻译（`compatibleWithImmersiveTranslate`）**  
  YouTube 字幕设置新增开关：开启后，当所选字幕为非中文（`languageCode` 不以 `zh` 开头）时，停止 VidBoost 自渲染并恢复原生字幕，让 Immersive Translate 渲染翻译结果；中文字幕仍由 VidBoost 渲染。含设置、i18n（中英）、tooltip 与说明文案。
- **[新增] Immersive Translate 检测工具**（`isImmersiveTranslateActive` / `isChineseLanguageCode`）  
  以 IT 字幕容器 `.imt-caption-container` 作为活跃信号（IT 全局 API 在隔离世界、`imt-state` 属性在 YouTube 不出现，故以容器为准）。

### Changed
- **字幕后处理改为按内容形态路由**（`postProcessSubtitles`）  
  废弃按 `track.kind === 'asr'` 强分支 + 标点判定的旧逻辑；改为 `needsResegment` 形状门（tiny / flash 占比、均长），精修短语走 `lightCleanFragments` 透传，碎 cue 才走 `refineAsrFragments` 重切。`detectSubtitleStyle` / `shouldUseAsrRefine` 同步统一到形状门。
- **原生字幕隐藏态收拢为幂等`setNativeSubtitlesHidden`**  
  统一在此之前散落的 `hideNativeSubtitles` / `showNativeSubtitles` 调用，并补上 Immersive Translate 字幕窗口（`#immersive-translate-caption-window`）的隐藏。
- **Immersive Translate 检测历次调整**  
  从「MAIN world 内联脚本检测」→「`imt-state` 属性」→ 最终「IT 渲染容器 + 语言码」；关闭不可靠的 IT 检测门，改为开启开关后非中文字幕一律让位 IT，容器在 VB 渲染中文时一并隐藏。
- **Popup 主页面紧凑布局**  
  头部从大标题区改为通栏窄条（`--popup-main-header-height`），整体间距收紧，并通过 `popup-main-density` scope 限定只影响主视图，其它视图与组件不受影响。
- 版本号升至 `1.9.0`（`package.json` / `package-lock.json` / `public/manifest.json`）。

### Fixed
- 修复开启兼容 IT 后，中文字幕仍被 VidBoost 隐藏的问题（隐藏规则现同时覆盖 IT 字幕容器）。
- 修复以 `track.kind` 判定导致「精修人工字幕被过度重切」与「ASR 被误判为精修」的两类误路由。

### Notes
- **IT 兼容默认关闭**；开启后仅影响非中文字幕路径，中文字幕功能不变。
- 相关 commits（`v1.8.5` → HEAD）：`54ebbb6`、`6480b75`、`28fc024`、`a48e5a0`、`39c7d2a`、`fad9020`、`d0f7ec7`、`f444462`、`8828d99`、`1e053ae`、`47369a2`、`104aab9`、`b97f0d3`、`bd8ec21`、`5a0b961`、`1ce24d8` 等。

## [1.8.5] - 2026-07-20

自 `v1.8.4` 以来，主线从 YouTube 字幕精炼转向 **通用 H5 倍速通杀**：落地 MAIN 世界轻量媒体内核（方案 E），让多数会回写倍速的网站也能稳住你设的速度；并补齐设置区分、文案与选择器 UI。

相对上一版 Release（`v1.8.4`）用户可感知的变化：

1. **更多网站能稳定调倍速** — 不再几乎只在 YouTube / B 站 / 抖音好用。
2. **新增「稳住倍速」三档** — 安全 / 均衡（默认）/ 强力，可按站点难度切换。
3. **设置更清晰** — 站点兼容与 Z/X/C 快捷键拆开，避免「到底在改什么」的困惑。

### Added
- **[新增] MAIN 媒体内核（`media-kernel`）**  
  在页面世界登记视频、选中主视频，并用 `HTMLMediaElement` 原生 descriptor 写入 `playbackRate` / seek，避免仅在扩展隔离世界改速后被站点立刻刷回。
- **[新增] Isolated ↔ MAIN 桥接（`MediaBridge`）**  
  H5 快捷键意图经 postMessage 交给内核；超时、失败时自动降级到原有 `VideoController` 直写，保证安全模式与注入失败时仍可用。
- **[新增] 稳住倍速三档（`h5_config.compatMode`）**  
  - **安全**：只改一次，最少干预（≈旧行为）。  
  - **均衡（默认 / 推荐）**：改速后短时（约 3s）守住，适合大多数会回写倍速的站。  
  - **强力**：持续守住当前倍速，直到你再次修改、换片或关闭功能。  
  设置项写入 content 可读配置（`h5_config` 进入 `CONTENT_SETTINGS_KEYS`），改完即可生效。
- **[新增] 全 frame 注入**  
  content script 与 media-kernel 开启 `all_frames: true`，改善 iframe / 嵌套播放器内视频的控制能力；无本地 video 时不抢快捷键。
- **[新增] 媒体内核纯逻辑单测与回归清单**  
  protocol / 选主视频 / sticky 策略单元测试；`docs/regression/h5-media-kernel-manual-checklist.md` 供手测 YT / B 站 / 抖音 / 其它站 / iframe。

### Changed
- **H5 倍速主路径改道**：兼容 / 强力模式下优先走 MAIN 内核；抖音仍 `ownsRate: false`，继续使用既有高倍速 guard，避免双重锁冲突。
- **H5 设置页信息架构**：将「站点兼容 / 稳住倍速」与「Z/X/C 快捷键」拆成独立区块。
- **兼容模式文案重写**（中英）：去掉难懂的 legacy / 页面世界术语，改为安全 / 均衡 / 强力及白话说明。
- **兼容模式控件**：原生 `<select>` 改为与 popup glass 主题一致的三段式 segment 选择器。
- 构建链路增加 `vite.mediakernel.config.ts`，产出 `assets/media-kernel.page.js`；Chrome 包校验同步要求该产物与 MAIN / `all_frames` 配置。
- 版本号升至 `1.8.5`（`package.json` / `package-lock.json` / `public/manifest.json`）。

### Fixed
- 修复通用站「能改一下倍速又被刷回 1x」的主因：隔离世界直写无法对抗站点 MAIN 逻辑。
- 修复 sticky 生命周期不同步：换片 / 卸视频 / 关闭控制 / 纠正次数封顶时正确 disarm，避免幽灵强锁。
- 修复桥接竞态：`ensureReady` 不会因 ping 超时把已就绪状态打回 false；kernel 晚启动时自动重推最近一次 configure。
- 修复 `all_frames` 下误抢键：presence 仅认本地 video；Enter 全屏无视频时不处理；seek 本地优先成功后再反馈，避免假 OSD / 双 seek。

### Notes
- **默认「均衡」**：升级后未单独改过配置的用户会使用新默认，以改善通杀体验；若遇异常可在 H5 设置里切回「安全」。
- **抖音**：通用内核不对 rate 强锁，高倍速仍走原有 guard。
- **第一期未做**：closed Shadow 强制 open、全局 `defineProperty` 反锁（L3）、截图 / 下载 / 滤镜等原版 h5player 大包能力。
- 相关 commits（`v1.8.4` → HEAD）：`d3aade2`、`748f812`、`eaa333b`、`1ac7136`、`29ec95c`、`4cf5a6f`、`7c23b3a`、`89a873f`、`3721432` 等。

## [1.8.4] - 2026-07-11

自 `v1.8.3` 以来，YouTube 自渲染字幕围绕 **ASR 自动字幕分段质量** 做了多轮迭代：先改进本地优化器参数，再保留 YouTube 原始时间边界，最后落地 **Scheme B+（先拆后并）** 并接到真正的生产渲染路径。

### Added
- **[新增] ASR 短句精炼器（Scheme B+）**（`processors/asr-merge.ts`）  
  专为 YouTube 自动字幕设计：在尽量对齐原始 timedtext 时间的前提下，让屏幕上更像完整短句。
  - **有标点 ASR**：先在 cue 内按 `. ? !` 等强句末切开（保护 `a.m.` / `p.m.` 等缩写），再合并未完成的相邻片段；句号结尾禁止再粘下一句。
  - **无标点 ASR**：按时间间隙（≤300ms）、词数上限（约 18 词）、时长上限（约 7s）、未完成尾巴词、pause words 等进行相邻合并。
  - **只合并不任意重切**（除有标点时的句内拆分外），时间取合并后首段 start / 末段 end。
  - 短孤儿片段（如 `box down`）、不完整尾巴（如 `I'm` / `my` / `to`）可适度放宽上限，减少半句悬挂。
- **[新增] ASR 精炼单元测试**（`processors/__tests__/asr-merge.test.ts`）  
  覆盖有标点拆句、无标点合并、缩写保护、大间隙不合并、噪声标签独立、CJK 合并等场景。

### Changed
- **[生产路径接线]** 在真正渲染入口 `YouTubeSubtitleOverlay.fetchTrackFragments` 中，parse 后按轨道类型后处理：
  - `track.kind === 'asr'` → `refineAsrFragments`（B+）
  - 人工字幕 → `optimizeSubtitles`
  - 此前 B+ 只改到 `SubtitleController`，而 content 实际使用的是 `YouTubeSubtitleOverlay`，导致 build 后界面仍无合并；本次修复该接线问题。
- **scrolling-asr 解析器**改为忠实于 YouTube 原始 event 边界：每个非 separator event 对应一个 fragment，保留 `tStartMs` / `dDurationMs`，避免 parser 层过大 `maxLength` 把短句揉成超长段。
- **CJK 工具与优化器常量对齐**：parser 层 `getMaxLength` 与优化器侧一致，减少解析阶段过度合并。
- **字幕优化器参数调优**（相对 1.8.3 初版）：停顿阈值、句末标点集合、无标点 ASR 检测与 pause-word 二次处理等，改善英文无标点自动字幕的可读性（人工字幕与优化器路径仍保留）。
- `SubtitleController` 与 overlay 对齐同一 ASR/人工字幕分支策略，避免测试路径与生产路径行为漂移。
- 版本号升至 `1.8.4`（`package.json` / `package-lock.json` / `public/manifest.json`）。

### Fixed
- 修复 scrolling-asr 使用过大 `maxLength` 导致自动字幕在解析阶段过度合并、丢失精确时间边界的问题。
- 修复「已实现 ASR 合并但界面无效」：合并逻辑未接入 `YouTubeSubtitleOverlay` 生产路径。
- 修复有标点 ASR 中「一句 cue 内粘两句」（如 `to the airport. I'm flying to Korea`）仅靠合并无法处理的问题：B+ 会先拆再并。
- 改善无标点 ASR 中半句悬挂（如 `from jet` / `lag.`）通过相邻合并拼成完整短语的能力。

### Notes
- `language = track.languageCode || 'en'` 仅表示缺少语言码时的默认值，**不是「仅英文启用」**；是否走 B+ 取决于 `track.kind === 'asr'`。
- 中间曾短暂采用「ASR 完全原样显示」以保留 YouTube 时间；1.8.4 最终在原样解析基础上增加 B+ 精炼，兼顾时间与可读性。
- 相关 commits（`v1.8.3` → HEAD）：`b7be56b`、`e037ebf`、`1399adf`、`3aec69d`。

## [1.8.3] - 2026-06-25

### Added
- **[新增] YouTube 字幕本地优化器**：实现基于规则的字幕分段优化，自动将逐字字幕合并为完整句子。
  - **英文字幕优化**：解决 YouTube 自动字幕逐字跳转问题，将单词级片段合并为句子级显示，片段数减少 70-90%。
  - **中文字幕优化**：改善 ASR 字幕按固定时间窗口切分的问题，按语义自然断句，片段数减少 30-50%。
  - **多语言支持**：自动检测中日韩（CJK）语言，针对性处理字符计数与文本合并逻辑。
  - **智能断句规则**：基于句末标点（. ? ! 。？！）、时间间隙（>1500ms）、长度上限、特殊标记（[、(、♪）和停顿词（and、but、so 等）进行断句。
  - **三遍优化流程**：第一遍按断句规则合并、质量检测触发停顿词检测、第三遍平衡重排到目标长度范围。
  - **性能保证**：O(n) 线性复杂度，典型视频（1000 片段）优化耗时 < 10ms，不影响播放性能。
  - **开箱即用**：无需用户配置，自动集成到字幕加载流程，出错时自动降级到原始字幕。
  - **测试覆盖**：新增 50+ 单元测试用例，覆盖基本合并、CJK 语言、时间间隙、特殊字符、边界情况等场景。

### Changed
- 优化 YouTube 自渲染字幕的加载流程，在抓取字幕后自动应用优化器处理。
- 改进字幕显示体验，字幕停留时间更长，跳动频率显著降低，语义完整性更好。

## [1.8.2] - 2026-06-21

### Changed
- **[重构] YouTube 字幕系统模块化重写**：将原有单一 298 行解析器文件重构为模块化架构，涵盖解析器层、UI 组件层和核心协调层。
  - **解析器层**：新增 5 种专用解析器（Standard、Karaoke、Stylized Karaoke、Scrolling ASR、Animated），支持智能格式检测、噪音过滤和 CJK 语言优化。
  - **UI 组件层**：基于 Shadow DOM 隔离的 Svelte 组件系统，包含可拖拽定位手柄、样式化文本显示和容器整合。
  - **核心层**：完整的 Fetcher（网络获取+缓存）、Controller（状态管理+事件系统）、Storage（持久化配置）和 Renderer（渲染协调）。
  - 代码结构：40 个模块化文件，~7,000 行代码（含测试），>300 个测试用例覆盖。
  - 质量保障：TypeScript strict mode、零运行时回归、Drop-in 替换验证。

## [1.8.1] - 2026-05-29

### Fixed
- 修复部分自定义 SVG 图标因内部 `fill` / `stroke` 固定颜色导致关闭态未完全变灰的问题。

## [1.8.0] - 2026-05-22

### Added
- 新增 YouTube 字幕状态记忆能力，打开其他视频时可恢复 CC 字幕开关状态。
- 新增字幕状态记忆范围设置，支持“全部视频共用”与“按频道分别记住”两种模式。
- 为 YouTube 视频下方进度条粗细设置新增直观预览，方便在 popup 中直接判断显示效果。

### Changed
- 重构 popup 功能选项页的通用设置样式，统一设置面板、字段容器、分段控件、标签列表与状态提示的视觉语言。
- 重排 YouTube 自渲染字幕样式面板，优化字体、颜色、效果和已导入字体管理的空间利用与信息层次。
- 优化 YouTube 会员视频屏蔽面板，将“全部屏蔽 / 仅屏蔽 / 仅允许”改为更清晰的分段选择，并重做频道名单编辑交互。
- 优化 Bilibili 自动打开中文字幕与指定画质相关面板，统一“全部视频 / 指定 UP 主”等范围切换和名单输入体验。
- 调整视频下方进度条粗细设置的表达方式，让数值调节与实际显示效果更容易对应。

### Fixed
- 修复禁用状态下部分 toggle 仍可通过点击或键盘触发的问题。
- 修复折叠面板展开动作与主开关状态切换耦合导致的误操作问题。
- 修复 YouTube 自渲染字幕在选择已导入字体后不会立即应用到当前样式的问题。

## [1.7.0] - 2026-05-15

### Added
- 新增 YouTube 视频下方进度条能力，在播放器底部提供一条精简进度提示，并支持调节进度条粗细。
- 新增 YouTube 底部进度条对应的 popup 配置入口，沿用现有 YouTube 设置区的主开关与展开式交互。
- 新增 YouTube 自渲染字幕与底部进度条相关功能截图，用于 README 和后续发布展示。

### Changed
- 调整 YouTube 底部进度条显示策略，仅在原生控制条自动隐藏后显示，原生控件重新出现时自动隐藏，直播场景下自动关闭该进度条。
- 补充 README 中关于 YouTube 自渲染字幕与视频下方进度条的功能说明，并同步更新展示截图。
- 补充 Firefox feature coverage 与 package 校验规则，确保新的 YouTube 底部进度条模块进入 Firefox 构建产物。
- 优化 release notes 生成逻辑：当 CHANGELOG 中存在当前版本条目时，优先直接使用该条目生成发布说明，并统一将完整更新链接指向当前 release 页面。
- 将版本号统一升级至 `1.7.0`。

## [1.6.3] - 2026-05-14

### Added
- 新增 GitHub Actions 自动发布流程，推送 `v*` tag 后可自动构建并创建 GitHub Release。
- 新增版本一致性校验脚本，保证 `package.json`、`package-lock.json`、`public/manifest.json` 保持同步。
- 新增自动 release notes 生成脚本，可基于 tag 区间整理版本变更。
- 新增 Chrome Release 打包脚本，统一生成 GitHub Release 附件。
- 新增仓库级 contributor guide 与 GitHub Actions 发版说明文档。

### Changed
- 优化 YouTube 自渲染字幕的全屏缩放逻辑，避免全屏时字体线性放大过头。
- 优化 YouTube 字幕拖拽手柄尺寸策略，避免跟随字幕一起过度放大。
- popup 中的版本显示兜底逻辑改为通用默认值，降低版本漂移风险。

## [1.6.2] - 2026-05-07

### Added
- 继续完善 YouTube 自渲染字幕与播放器增强体验，让字幕样式面板、显示效果和相关控件更稳定。

### Fixed
- 修复 `屏蔽会员视频` 失效问题，恢复 YouTube 会员视频过滤能力。

### Changed
- 优化 YouTube 自渲染字幕与 CDN 控件显示体验，让字幕样式与相关控件交互更稳定。

## [1.6.1] - 2026-04-25

### Added
- 新增 YouTube 自渲染字幕能力，由 VidBoost 按当前播放进度自行渲染字幕，重点改善高倍速场景下的字幕同步体验。
- 新增字幕位置拖动能力，支持直接调整字幕显示位置。
- 新增字幕样式自定义能力，支持字体、字号、字重、文字颜色、透明度、背景颜色、背景透明度与背景圆角。
- 新增字符边缘效果，支持描边、投影、浮雕、压印，并支持多效果叠加。
- 新增字幕字体导入能力，支持 `.ttf / .otf / .woff / .woff2` 导入后直接用于字幕渲染。

### Fixed
- 修复字幕拖动不可用的问题，并优化拖动手柄的显示 / 隐藏交互。
- 优化播放器点击命中逻辑，减少 YouTube 场景下 `fast pause` 的误触问题。
- 修复部分暂停逻辑的误触场景。

### Changed
- 增强 YouTube 字幕字体自定义能力。
- 优化字幕边缘样式渲染与样式面板交互。
- 增加自渲染失败时自动回退到 YouTube 原生字幕的稳定性逻辑。

## [1.5.0] - 2026-04-02

### Added
- 新增 Bilibili “指定画质”能力，可为特定 UP 主设置默认目标画质。

### Changed
- 调整 Bilibili 画质相关文案与默认行为，让按 UP 主区分的画质策略更清晰。

## [1.4.1] - 2026-03-31

### Added
- 为 h5player 设置新增 `Z / X / C` 快捷键总开关，允许用户自行决定是否启用这组倍速快捷键。
- 补齐 Firefox 兼容与基础发布链路相关能力。

### Fixed
- 修复 YouTube CDN 状态组件在页面重挂载后的布局问题。

### Changed
- 更新 Firefox 安装说明与相关 README 文案。

## [1.4.0] - 2026-03-20

### Added
- 新增 YouTube 当前 CDN 国家显示，并支持跟随插件语言切换显示国家名称。
- 新增统计面板网速换算，支持 YouTube 和 Bilibili。

### Changed
- 优化 popup 页脚链接与图标样式。
- 新增 GitHub 图标入口，并让版本号跳转到 Releases。
- 统一弹窗功能图标样式，整体视觉更一致。
- 更新 README、安装说明与功能截图。

## [1.3.6] - 2026-03-18

### Changed
- 优化自动中文字幕相关 UI 与交互细节，让入口与设置面板更直观。

## [1.3.5] - 2026-03-17

### Fixed
- 修复开启“禁用双击暂停”后部分点击元素不生效的问题。
- 修复 Bilibili 弹幕框等交互元素在相关场景下受影响的问题。

## [1.3.4] - 2026-03-15

### Fixed
- 修复抖音倍速不生效问题，恢复高倍速守护能力。

## [1.3.3] - 2026-03-15

### Added
- 新增哔哩哔哩自动开启中文字幕功能，并优先面向常见中文字幕场景。

## [1.3.2] - 2026-03-14

### Added
- 为 CDN 测速增加设置界面，便于直接在插件面板里管理测速与节点选择。
- 新增 YouTube 原始音频优先能力。
- 完成弹出界面的国际化，并改进功能生命周期管理。
- 完成自动暂停的后台播放仲裁与焦点覆盖相关优化。
- 为 YouTube 会员视频黑名单 / 白名单补充更完整的交互能力。
- 增补多组功能截图与 README 说明。

### Fixed
- 修复自动暂停相关稳定性问题。

## [1.2] - 2026-03-01

### Added
- 新增 YouTube 会员视频屏蔽能力。
- 新增哔哩哔哩 CDN 切换能力。
- 新增禁用空格滚动，避免空格暂停时页面跟随滚动。
- 新增数字键 `1-6` 直切倍速能力。

### Fixed
- 禁止拦截 YouTube 全屏评论等特定元素，减少误伤交互。
- 静音视频不再抢占自动暂停相关判断。

### Changed
- 优化 OSD 提示框 UI。
- 做了一轮结构层面的整理，为后续扩展能力铺路。

## [1.1] - 2026-02-16

### Changed
- 自动暂停逻辑增强，忽略小动画、SponsorBlock、章节预览等不应触发暂停的场景。
- 优化 Bilibili 预览与 Google 预览等特殊场景下的稳定性。
- 补充隐私政策与 README 说明。

### Fixed
- 修复若干扩展稳定性问题。

## [1.0] - 2026-02-06

### Added
- 初始稳定版本发布，完成基础 UI 与核心视频增强能力整合。
- 初步形成统一的视频增强入口与基础交互体验。

### Changed
- 完成品牌统一并切换为 `VidBoost` 命名。
