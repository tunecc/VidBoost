...
VidBoost 是一个给 Chrome / Edge / Firefox 用的视频增强扩展。我把自己长期在用、也最容易分散到各类脚本里的几项功能，收进了同一个入口。

如果你平时会装好几个油猴脚本来处理倍速、后台暂停、误触全屏，或者 YouTube / B 站的一些小毛病，这个扩展就是想把这些零散能力整理好，做成一个更稳定的 Manifest V3 方案。
...
- 常用功能放在一起，少在多个脚本和设置页之间来回切。
- 尽量避开脚本互相冲突、设置分散、失效后还得重启浏览器这类麻烦。
- 先解决真正高频的场景：倍速、后台暂停、误触全屏，以及 YouTube 和 B 站的一些老问题。
- 逻辑按需激活。页面里没视频，就尽量不做多余的事。
...
如果只是日常使用，Firefox 更建议直接装上面的 AMO 版本；README 里的 Firefox 统计数据也会跟着 AMO 徽章自动更新。
