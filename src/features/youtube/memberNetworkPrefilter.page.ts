...
import {
    ...
    MEMBER_NETWORK_PREFILTER_GLOBAL_KEY,
...
(() => {
    ...
    if (host[MEMBER_NETWORK_PREFILTER_GLOBAL_KEY]) return;
    host[MEMBER_NETWORK_PREFILTER_GLOBAL_KEY] = true;
    ...
    window.addEventListener('message', (event: MessageEvent<unknown>) => {
        ...
        if (data.channel !== MEMBER_NETWORK_PREFILTER_BRIDGE_CHANNEL) return;
        if (data.type !== 'initial' && data.type !== 'change') return;
        updateConfig(data.config);
    });

    window.postMessage({
        source: MEMBER_NETWORK_PREFILTER_PAGE_SOURCE,
